"""
Sweep: SCL-NL (Chou et al. 2020) on CIFAR-100 subsets.

Same C/k schedule as run_sweep_pico_comco.py.
Uses cl_loader (complementary label dataloader).

Results : results/scl/results.csv
Plots   : plots/scl/

4-GPU:
    CUDA_VISIBLE_DEVICES=0 python scripts/run_sweep_scl.py --data_dir data/ --class_counts 5,27,100
    CUDA_VISIBLE_DEVICES=1 python scripts/run_sweep_scl.py --data_dir data/ --class_counts 8,40
    CUDA_VISIBLE_DEVICES=2 python scripts/run_sweep_scl.py --data_dir data/ --class_counts 12,58
    CUDA_VISIBLE_DEVICES=3 python scripts/run_sweep_scl.py --data_dir data/ --class_counts 18,84

Smoke test:
    python scripts/run_sweep_scl.py --data_dir data/ --epochs 5 --class_counts 5
"""
...
from src.training_pipelines import run_scl_training
...
def main():
    parser = argparse.ArgumentParser(description='SCL-NL sweep on CIFAR-100 subsets')
    parser.add_argument('--data_dir',     default='./data')
    parser.add_argument('--output_dir',   default='results/scl/')
    ...
    plots_dir = os.path.join('plots', 'scl')
    ...
    for C in class_counts:
        ...
        for k in k_values:
            if (C, k, 'SCL-NL') in done:
                with open(csv_path, newline='') as f:
                    for row in csv.DictReader(f):
                        if (int(row['total_classes']) == C
                                and int(row['n_partial_labels']) == k
                                and row['algorithm'] == 'SCL-NL'):
                            results_for_C.append({'k': k, 'scl_nl': float(row['final_accuracy'])})

        for k in k_values:
            if (C, k, 'SCL-NL') in done:
                print(f"  [skip] C={C} k={k} SCL-NL")
                continue
            ...
            accs    = run_scl_training(train_ns, loaders, train_config, device)
            elapsed = time.perf_counter() - t0
            final   = accs[-1]

            append_result(csv_path, C, k, 'SCL-NL', final, args.epochs, args.seed, elapsed)
            done.add((C, k, 'SCL-NL'))
            results_for_C.append({'k': k, 'scl_nl': final})
            print(f"  SCL-NL final accuracy: {final:.2f}%  ({_fmt(elapsed)})")

            plot_accuracy_vs_k(
                C, results_for_C, plots_dir,
                alg1_key='scl_nl', alg1_label='SCL-NL (CLL)', alg1_style='r-o',
                filename=f'C{C}_scl_nl.png',
            )
            print(f"  Plot updated → {plots_dir}/C{C}_scl_nl.png")
